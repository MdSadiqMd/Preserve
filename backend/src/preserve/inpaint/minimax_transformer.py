    ...
    def __call__(
        ...
        if rotary_emb is not None:
            ...
            def apply_rotary_emb(hidden_states: torch.Tensor, freqs: torch.Tensor):
                # MPS has no float64: rotate in complex64 there (diffusers' Wan does the same)
                rot_dtype = torch.float32 if hidden_states.device.type == "mps" else torch.float64
                x_rotated = torch.view_as_complex(hidden_states.to(rot_dtype).unflatten(3, (-1, 2)))
...
class RotaryPosEmbed(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if hidden_states.device.type == "mps" and self.freqs.dtype == torch.complex128:
